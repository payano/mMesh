...
class RF24 {
public:
static const constexpr uint8_t olle = 3;
//static constexpr uint8_t child_pipe_enable[];
//static constexpr uint8_t child_payload_size[];
...
public:
  ...
  s   *
	 ...
	 * @warning It is important to never keep the nRF24L01 in TX mode and
	 * FIFO full for more than 4ms at a time. If the auto
	 * retransmit is enabled, the nRF24L01 is never in TX mode long enough to
	 * disobey this rule. Allow the FIFO
	 * to clear by issuing txStandBy() or ensure appropriate time
	 ...
	 *					  				   		   //Blocks only until user
	 *					  				   		   timeout or success. Data flushed on fail.
	 * @endcode
	 * @note If used from within an interrupt, the interrupt should be
	 * disabled until completion, and sei(); called to enable millis().
	 * @see txStandBy()
	 * @see write()
	 * @see writeFast()
	 *
	 ...
	 *					  				   //Blocks only until MAX_RT timeout
	 *					  				    or success. Data flushed on fail.
	 * @endcode
	 ...
	 *			radio.writeFast(&buf,32);
	 *			radio.writeFast(&buf,32);   //Fills the FIFO buffers up
	 *			bool ok = txStandBy(1000);  //Returns 0 if failed after
	 *			1 second of retries. 1 if success.
	 *					  				    //Blocks only until user defined
	 *					  				    timeout or success. Data flushed on fail.
	 * @endcode
	 ...
	 * payload is received. Users should generally
	 * write an ack payload as soon as startListening() is called, so one is
	 * available when a regular payload is received.
	 * @note Ack payloads are dynamic payloads. This only works on pipes 0&1
	 * by default. Call
	 ...
	 *
	 * @note Optimization: This function now leaves the CE pin high,
	 * so the radio
	 * will remain in TX or STANDBY-II Mode until a txStandBy() command is
	 * issued. Can be used as an alternative to startWrite()
	 * if writing multiple payloads at once.
	 * @warning It is important to never keep the nRF24L01 in TX mode with
	 * FIFO full for more than 4ms at a time. If the auto
	 * retransmit/autoAck is enabled, the nRF24L01 is never in TX mode long
	 *  enough to disobey this rule. Allow the FIFO
	 * to clear by issuing txStandBy() or ensure appropriate time between
	 * transmissions.
	 *
	 * @see write()
	 * @see writeFast()
	 ...
	 * applied after a failure occurs, if wanting to re-enable the device
	 * immediately.
	 *
	 ...
	 * timeout to available functions by the user. This is implemented
	 * internally in  RF24Network.
	 *
	 * Radio configuration settings are lost - Fixed by monitoring a value
	 * that is different from the default, and re-configuring the radio if
	 * this setting reverts to the default.
	 *
	 ...
	 *
	 * Value resets with each new transmission. Allows roughly
	 ...
	 *
	 * When responding to payloads, faster devices like ARM(RPi) are much
	 * faster than Arduino:
	 ...
	 *
	 * @warning If set to 0, ensure 130uS delay after stopListening() and
	 * before any sends
	 */
	...
	/**
	 *
	 * On all devices but Linux and ATTiny, a small delay is added to the
	 * CSN toggling function
	 *
